...
int BFS(int x , int y, int n , int m) {
    ...
    while (!q.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            int xx = curx + dx[k];
            int yy = cury + dy[k];
            if (xx < n && yy < m && xx >= 0 && yy >= 0 && vis[xx][yy] == 0 && (g[xx][yy] == '.' || g[xx][yy] == 'B')) {
                vis[xx][yy] = 1;
                dis[xx][yy] = dis[curx][cury] + 1;
                par[xx][yy] = make_pair(curx , cury);
                //cout << "from ";
                //cout << par[xx][yy].first << " " << par[xx][yy].second;
                //cout << " To " << xx << " " << yy << "\n";
                if (k == 0) {
                    path[xx][yy] = 'R';
                } else if (k == 1) {
...
void PathPrint(int x , int  y) {
    while (x != -1 && y != -1) {
        ...
        int tmpx = par[x][y].first;
        int tmpy = par[x][y].second;
        //cout << tmpx << " " << tmpy << "\n";
        x = tmpx;
        y = tmpy;
    }
