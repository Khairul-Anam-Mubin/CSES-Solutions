...
const int mxN = 2e5 + 1;
vector <pair <ll , ll>> g[mxN] , gr[mxN];
vector <ll> cost(mxN);
void Dijkstra(ll st , ll ed, int typ = 0) {
    cost.assign(mxN, INF18);
    cost[st] = 0;
...
int main() {
    ...
    ll ans = INF18;
    for (auto it : edges) {
        if (cost1[it.first] != INF18 && cost2[it.second.first] != INF18) {
            ans = min(ans , ll(cost1[it.first]  + cost2[it.second.first] + it.second.second / 2));
        }
    }
    cout << ans << "\n";
    return 0;
